...
- `main-vulnerable`: contiene 9 vulnerabilidades intencionadas para poder explotarlas en local.
- `main-secure`: implementa exactamente las mismas funcionalidades, pero corregidas con buenas prácticas (consultas parametrizadas, escape de salida, `execFile`, `bcrypt`, `helmet`, etc.).

Las capturas de explotación están en [docs/evidence/vulnerable/](docs/evidence/vulnerable/).
...
La aplicación queda disponible en `http://localhost:3000`. Los usuarios de prueba son `admin / admin` y `user / user`. En esta rama las contraseñas se guardan en texto plano a propósito.
...
Todas las capturas se han tomado en `main-vulnerable` ejecutándose en local.

| Nº | Vulnerabilidad | Payload o prueba | Captura |
|----|----------------|------------------|---------|
...
| 3 | Stored XSS | Publicar un comentario en `/guestbook` con `<img src=x onerror=alert(1)>` o `<script>alert(1)</script>`. El alert salta cada vez que se abre la página. | [03-stored-xss.png](docs/evidence/vulnerable/03-stored-xss.png) |
| 4 | Command Injection | En `/tools/ping`, host `127.0.0.1 & whoami` (Windows) o `127.0.0.1; whoami` (Linux/macOS). Aparece la salida de `whoami` después del ping. | [04-command-injection.png](docs/evidence/vulnerable/04-command-injection.png) |
...
| 6 | Broken Authentication | Tras iniciar sesión, abrir DevTools y ejecutar `document.cookie` en consola. Se ve `connect.sid`, lo que confirma que la cookie no tiene `HttpOnly`. | [06-broken-authentication.png](docs/evidence/vulnerable/06-broken-authentication.png) |
| 7 | Broken Access Control (IDOR) | Iniciar sesión como `user` (id 2) y abrir `/users/1`. Se muestran los datos del admin sin error. | [07-broken-access-control-idor.png](docs/evidence/vulnerable/07-broken-access-control-idor.png) |
| 8 | Security Misconfiguration | Abrir `/debug/error`: aparecen mensaje y stack trace completos. Con `curl -I http://localhost:3000/` se ve la cabecera `X-Powered-By: Express`. | [08-security-misconfiguration.png](docs/evidence/vulnerable/08-security-misconfiguration.png) |
| 9 | Sensitive Data Exposure | Iniciar sesión y abrir `/debug`. La página muestra la contraseña en texto plano, el session id, un token simulado y datos internos del usuario y de la app. | [09-sensitive-data-exposure.png](docs/evidence/vulnerable/09-sensitive-data-exposure.png) |
...
docs/evidence/          # capturas de explotación
```
